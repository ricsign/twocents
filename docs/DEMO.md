...
- [ ] Press **RESET DEMO** on the town screen. It reseeds the session and puts
      ...
      pre-briefed, Richard's seat empty again, and no run started — a room whose
...
| 0:15 | The Briefing | PRESS START → `/brief`. Type Richard's brief live: somewhere warm, **$600 max, don't tell them**, nothing before 8am. Point at the "your agent knows" panel as the budget line lands with a lock on it. | A human telling an agent the thing they would not type in the group chat. |
| 0:35 | The Personality | Next → `/personality`. Read the opening preview line first — it is already arguing Richard's brief against Will's Cancun — then drag to diplomatic-but-firm and read it again. | The sliders change the sentence under them. The agent is going to sound like him. |
| 0:45 | The Town | Next → `/town`. Tap **2x** as the first bubble lands — it restarts the stream, which at that point is one line. Then say nothing for ten seconds: "Richard's agent is steering away from Cancun. It has never said why, and it never will." | Four characters at a table. Speech bubbles. A transcript filling on the right. |
| 1:30 | The Plan | SEE THE PLAN → `/plan`. Read the fairness meter: "Will gave up the resort. Nobody was overruled — that is computed, not claimed." Then the private report: "$540 a head, $60 under your number. Nobody heard your number." Tap APPROVE. | One plan, a runner-up, four bars, and a note only Richard can see. |
...
| 2:00 | The flip | Back to `/personality` — the step-2 pip in the top bar is a link — drag Richard from easygoing to stubborn, then press **SEND MY AGENT TO THE TOWN**. Tap 4x. | A different argument and a different plan, from one slider. Moving the slider cleared the old plan, so the town runs itself the moment you arrive; the only button you press is the one that took you there. |
| 2:20 | Judges' round | JUDGES' ROUND — the gold button is on the town screen and on the plan screen → `/judges`. Hand over the keyboard. Four names, four one-line wants, four private budgets — or just press MAKE THE ROOM, it is prefilled with a dinner. **It now makes a room rather than overwriting the demo**, so you land on the join screen: pick which of the four you are, and pass a phone the code if you want a second seat in the room. Works with or without a key. | Their dinner, argued by four agents, in about forty seconds. |
...
has seen Richard say "$600, don't tell them" with their own eyes. When his agent
...
and the briefs and personalities come back seeded, Richard's empty. It takes about
